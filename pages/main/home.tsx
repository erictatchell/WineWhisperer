      ...
      <div className="grid justify-center mb-40">
        {showExamples && (  // conditional rendering based on showExamples state
          <div className='mx-6'>
            ...
            <div onClick={() => { setPageDesc('A medium-bodied red with complex flavours of dark fruits and hints of oak'); setDescription('A medium-bodied red with complex flavours of dark fruits and hints of oak'); handleSubmit; }} onChange={handleDescriptionChange} className="grid font-semibold grid-cols-4 p-2 backdrop-blur-md text-dijon/95 max-w-sm ml- mr- mb-6 border border-dijon/95 rounded-lg backdrop-blur-sm shadow bg-brendan/50 sm:max-w-full">
        ...
        {!loading && wines.length != 0 ?
          <div className='text-center'>
            <div className='text-5xl mt-2 text-black '>“”</div>
            <div className=' mx-10 mb-5 font-medium text-black '>{pageDesc}</div>
          </div>
          : <div></div>}
        {!loading ?
        ...
        <div className="max-w-lg mx-auto p-4 mb-16 bg-black/10 backdrop-blur-md">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2 flex flex-col md:flex-row items-start md:items-center">
              <input required value={description} onChange={handleDescriptionChange} className="text-sm block w-full border-dijon/95 rounded-lg bg-transparent border p-2.5 dark:text-black hover:bg-black focus:bg-gray-200 focus:bg-black focus:outline-none mt-2 md:mt-0 md:ml-2" id="wine-description" type="text" placeholder='Type here' />
            </div>
